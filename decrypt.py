...
def main():
	ifname = sys.argv[1]
	ofname = sys.argv[2]
	cipheredImage, header = read_bmp_image(ifname)
	width = len(cipheredImage)
	height =len(cipheredImage[0])
	...
	phl = [[0 for i in range(height)] for j in range(width)]
	for i in range(math.ceil(width*height/3)):
		...
		phl[rows[0]][cols[0]] = cipheredImage[rows[0]][cols[0]]^xx[xbar//2+1]
		if i<math.ceil(width*height/3)-1:
			phl[rows[1]][cols[1]] = cipheredImage[rows[1]][cols[1]]^xx[(xbar+8)//3]
			phl[rows[2]][cols[2]] = cipheredImage[rows[2]][cols[2]]^xx[xbar//3]
		else:
	...
	while len(h_list)<width:
		...
		if hi not in h_list:
			h_list.append(hi)
		xn1 = logistic_map(xn1)
	pl = unscramble_rows(phl, h_list)

	plainImage = [[] for i in range(width)]
	for i in range(width):
		...
		while len(l_list)<height:
			...
			if li not in l_list:
				l_list.append(li)
			xn1 = logistic_map(xn1)
		plainImage[h_list[i]] = unscramble_cols(pl[h_list[i]], l_list)
...
def unscramble_rows(matrix, hl):
	'''
	Desembaralha as linhas de matrix considerando que elas foram embaralhadas na ordem
	estabelecida por hl e retorna matrix desembarahada.
	'''
	ret = [[] for i in range(len(matrix))]
	for i in range(len(matrix)):
		ret[hl[i]] = matrix[i]
	return ret

def unscramble_cols(row, ll):
	'''
	Desembaralha as colunas da linha row considerando que elas foram embaralhadas na ordem
	estabelecida por ll e retorna row desembaralhada.
	'''
	ret = [0 for i in range(len(row))]
